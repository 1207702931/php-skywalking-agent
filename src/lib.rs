...
fn random(arguments: &mut [ZVal]) -> phper::Result<String> {
    // Get the first argument, expect the type `ZStr`, and convert to Rust utf-8
    // str.
    let prefix = arguments[0].expect_z_str()?.to_str()?;
    // unsafe {sys::api_module.putenv_handler("abc=1", 6)};
    // Macro which do php internal `echo`.
    let string = format!("{}-{}", prefix, RandomGenerator::generate());
...
pub fn get_module() -> Module {
    ...
    module.add_function("random_str", random);
